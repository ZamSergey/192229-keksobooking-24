...
const TEST_TITLES = ['Гранд Будапешт', 'Три поросенка', 'Царский Двор', 'Parck Inn', 'У Петровича'];
const TEST_TYPES = ['palace', 'flat', 'house', 'bungalow' ,'hotel'];
const TEST_CHECKIN = ['12:00', '13:00', '14:00'];
const TEST_FEATURES = ['wifi', 'dishwasher', 'parking', 'washer', 'elevator', 'conditioner'];
const TEST_DESCRIPTIONS = ['Невероятно чистое помещение','Много траканов','Тонкие стены. Все слышно', 'Стеклянные стены'];
...
for(let i = 1; i<=10;i++){
  TEST_AVATARS[i] = i === 10 ? `img/avatars/user${  i  }.png` : `img/avatars/user0${  i  }.png`;
}
//Перемешиваю массив случайным образом, чтобы аватарки не шли подряд
shuffle(TEST_AVATARS);
...
const getHotel = (numberOfHotel) => {
  ...
  return {
    author: {
      avatar: TEST_AVATARS[numberOfHotel],
    },
    offer: {
      title: TEST_TITLES[getRandomPositiveInteger(0, TEST_TITLES.length - 1)],
      address: `${location.lat }, ${ location.lng}`,
      price: getRandomPositiveInteger(10, 1000),
      type: TEST_TYPES[getRandomPositiveInteger(0, TEST_TYPES.length - 1)],
      ...
      features: features,
      description: TEST_DESCRIPTIONS[getRandomPositiveInteger(0, TEST_DESCRIPTIONS.length - 1)],
      photos: photos,
    },
    location: location,
  };
